...
#ifndef UNDEROCEAN_LEVEL6_H
#define UNDEROCEAN_LEVEL6_H
...
public:
    explicit Level6(GameServer& server): LevelBase(server) {
        initialize();
    }
    ...
    int getLevel() override { return 6; }
protected:
...
inline void Level6::onPlayerLeave(PlayerData& playerData) {
    auto& peerEntities = coordinator_.getEntitiesWith(networkSignature_);
    for (Entity e : peerEntities) {
...
inline void Level6::onPlayerJoin(PlayerData& playerData) {   //call by GameServer
...
inline void Level6::update(float dt) {
    LevelBase::update(dt);
}
#endif //UNDEROCEAN_LEVEL6_H