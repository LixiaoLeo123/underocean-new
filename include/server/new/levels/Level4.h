...
#ifndef UNDEROCEAN_LEVEL4_H
#define UNDEROCEAN_LEVEL4_H
...
public:
    explicit Level4(GameServer& server): LevelBase(server) {
        initialize();
    }
    ...
    [[nodiscard]] int getLevelID() const override { return 4; };
    ...
    int getLevel() override { return 4; }
protected:
...
inline void Level4::onPlayerLeave(PlayerData& playerData) {
    auto& peerEntities = coordinator_.getEntitiesWith(networkSignature_);
    for (Entity e : peerEntities) {
...
inline void Level4::onPlayerJoin(PlayerData& playerData) {   //call by GameServer
...
inline void Level4::update(float dt) {
    LevelBase::update(dt);
}
#endif //UNDEROCEAN_LEVEL4_H