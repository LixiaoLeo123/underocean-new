...
#ifndef UNDEROCEAN_LEVEL5_H
#define UNDEROCEAN_LEVEL5_H
...
public:
    explicit Level5(GameServer& server): LevelBase(server) {
        initialize();
    }
...
inline void Level5::onPlayerLeave(PlayerData& playerData) {
    auto& peerEntities = coordinator_.getEntitiesWith(networkSignature_);
    for (Entity e : peerEntities) {
...
inline void Level5::onPlayerJoin(PlayerData& playerData) {   //call by GameServer
...
inline void Level5::update(float dt) {
    LevelBase::update(dt);
}
#endif //UNDEROCEAN_LEVEL5_H