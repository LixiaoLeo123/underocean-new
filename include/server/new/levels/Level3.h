...
#ifndef UNDEROCEAN_LEVEL3_H
#define UNDEROCEAN_LEVEL3_H
...
public:
    explicit Level3(GameServer& server): LevelBase(server) {
        initialize();
    }
    ...
    int getLevel() override { return 3; }
protected:
...
inline void Level3::onPlayerLeave(PlayerData& playerData) {
    auto& peerEntities = coordinator_.getEntitiesWith(networkSignature_);
    for (Entity e : peerEntities) {
...
inline void Level3::onPlayerJoin(PlayerData& playerData) {   //call by GameServer
...
inline void Level3::update(float dt) {
    LevelBase::update(dt);
}
#endif //UNDEROCEAN_LEVEL3_H