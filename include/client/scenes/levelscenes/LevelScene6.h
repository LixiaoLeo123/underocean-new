...
#ifndef UNDEROCEAN_LEVELSCENE6_H
#define UNDEROCEAN_LEVELSCENE6_H
#include "LevelSceneBase.h"
#include "server/new/component/Components.h"

class LevelScene6 : public LevelSceneBase {
public:
    explicit LevelScene6(const std::shared_ptr<ClientNetworkDriver>& driver, ClientCommonPlayerAttributes& playerAttributes, const std::shared_ptr<ChatBox>& chatBox)
        ...
        // }
        background_.setTexture(ResourceManager::getTexture("images/backgrounds/bg6.png"));
        writer_.writeInt8(static_cast<std::uint8_t>(6));  //to level 1
        driver_->send(writer_.takePacket(), 0, ServerTypes::PacketType::PKT_LEVEL_CHANGE, 1);
        writer_.clearBuffer();
    }

protected:
    int getLevelNum() override { return 6; };
...
private:
    static constexpr UVector MAP_SIZE{1728.f, 324.f};  //decided by bg
    static constexpr int CHUNK_ROWS = 108;   //about 16 x 16 px  15, 26
    static constexpr int CHUNK_COLS = 21;
    enum class State {
        ANIMATION,
        GAMING
    };
...
#endif //UNDEROCEAN_LEVELSCENE6_H