#include <array>
#include "common/utils/IntegritySecret.h"
#include "common/utils/Integrity.h"

#include <vector>

static void xorScramble(std::uint8_t *data, std::size_t size) {
    for (std::size_t i = 0; i < size; ++i) {
        data[i] ^= INTEGRITY_SECRET[i % INTEGRITY_SECRET_LEN];
    }
}
namespace {
    constexpr std::uint32_t K[64] = {
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    };
    std::uint32_t rotr(std::uint32_t x, std::uint32_t n) {
        return (x >> n) | (x << (32 - n));
    }
    void sha256Transform(std::uint32_t state[8], const std::uint8_t block[64]) {
        std::uint32_t w[64];
        for (int i = 0; i < 16; ++i)
            w[i] = (block[i * 4] << 24) | (block[i * 4 + 1] << 16) | (block[i * 4 + 2] << 8) | block[i * 4 + 3];
        for (int i = 16; i < 64; ++i) {
            std::uint32_t s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >> 3);
            std::uint32_t s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >> 10);
            w[i] = w[i - 16] + s0 + w[i - 7] + s1;
        }
        std::uint32_t a = state[0], b = state[1], c = state[2], d = state[3];
        std::uint32_t e = state[4], f = state[5], g = state[6], h = state[7];
        for (int i = 0; i < 64; ++i) {
            std::uint32_t S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
            std::uint32_t ch = (e & f) ^ ((~e) & g);
            std::uint32_t temp1 = h + S1 + ch + K[i] + w[i];
            std::uint32_t S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
            std::uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
            std::uint32_t temp2 = S0 + maj;
            h = g;
            g = f;
            f = e;
            e = d + temp1;
            d = c;
            c = b;
            b = a;
            a = temp1 + temp2;
        }
        state[0] += a;
        state[1] += b;
        state[2] += c;
        state[3] += d;
        state[4] += e;
        state[5] += f;
        state[6] += g;
        state[7] += h;
    }
    std::array<std::uint8_t, 32> sha256(const std::uint8_t *data, std::size_t len) {
        std::array<std::uint8_t, 32> hash{};
        std::uint32_t state[8] = {
            0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
            0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
        };
        std::size_t full_blocks = len / 64;
        for (std::size_t i = 0; i < full_blocks; ++i) {
            sha256Transform(state, data + i * 64);
        }
        std::uint8_t last_block[128] = {};
        std::size_t rem = len % 64;
        std::memcpy(last_block, data + full_blocks * 64, rem);
        last_block[rem] = 0x80;
        if (rem >= 56) {
            sha256Transform(state, last_block);
            std::memset(last_block, 0, 64);
        }
        std::uint64_t bit_len = len * 8;
        for (int i = 0; i < 8; ++i) last_block[63 - i] = bit_len >> (i * 8);
        sha256Transform(state, last_block);
        for (int i = 0; i < 8; ++i) {
            hash[i * 4 + 0] = (state[i] >> 24) & 0xFF;
            hash[i * 4 + 1] = (state[i] >> 16) & 0xFF;
            hash[i * 4 + 2] = (state[i] >> 8) & 0xFF;
            hash[i * 4 + 3] = state[i] & 0xFF;
        }
        return hash;
    }
    std::array<std::uint8_t, 32>
    hmac_sha256(const std::uint8_t *key, std::size_t key_len,
                const std::uint8_t *data, std::size_t data_len) {
        constexpr std::size_t BLOCK_SIZE = 64;
        std::uint8_t k0[BLOCK_SIZE]{};
        if (key_len > BLOCK_SIZE) {
            auto h = sha256(key, key_len);
            std::memcpy(k0, h.data(), 32);
        } else {
            std::memcpy(k0, key, key_len);
        }
        std::uint8_t ipad[BLOCK_SIZE];
        std::uint8_t opad[BLOCK_SIZE];
        for (std::size_t i = 0; i < BLOCK_SIZE; ++i) {
            ipad[i] = k0[i] ^ 0x36;
            opad[i] = k0[i] ^ 0x5c;
        }
        // inner = SHA256(ipad || data)
        std::vector<std::uint8_t> inner_buf(BLOCK_SIZE + data_len);
        std::memcpy(inner_buf.data(), ipad, BLOCK_SIZE);
        std::memcpy(inner_buf.data() + BLOCK_SIZE, data, data_len);
        auto inner = sha256(inner_buf.data(), inner_buf.size());
        // outer = SHA256(opad || inner)
        std::uint8_t outer_buf[BLOCK_SIZE + 32];
        std::memcpy(outer_buf, opad, BLOCK_SIZE);
        std::memcpy(outer_buf + BLOCK_SIZE, inner.data(), 32);
        return sha256(outer_buf, BLOCK_SIZE + 32);
    }
}
namespace Integrity {
    std::array<std::uint8_t, 32> hash(const std::uint8_t *data, std::size_t size) {
        return hmac_sha256(INTEGRITY_SECRET, SHA_SECRET_LEN, data, size);
    }
    void protect(std::uint8_t* data, std::size_t size) {
        xorScramble(data, size);
    }
    bool unprotect(std::uint8_t *data, std::size_t size) {
        xorScramble(data, size);
        return true;
    }
}
